...
export default function Calculator() {
    ...
    // Dummy calculation based on your previous plan data
    const calculateProfit = () => {
        // contoh sederhana
        const dailyROI = 13.8; // persentase daily
        const totalProfit = (dailyROI * days) / 100;
        const earnings = amount * (1 + totalProfit / 100);
        return { dailyROI, totalProfit, earnings };
    };

    const { dailyROI, totalProfit, earnings } = calculateProfit();

    const incrementAmount = () => setAmount((prev) => prev + 0.01);
    const decrementAmount = () => setAmount((prev) => Math.max(prev - 0.01, minAmount));
                ...
                <div className="input-group">
                    ...
                    <div className="input-controls">
                        ...
                        <div className="input-buttons">
                            <button className="input-btn" onClick={decrementAmount}><FaMinus /></button>
                            <button className="input-btn" onClick={incrementAmount}><FaPlus /></button>
                            <button className="input-btn max-btn" onClick={setMaxAmount}>MAX</button>
                        </div>
                    </div>
                ...
                <div className="profit-display">
                    <div className="profit-grid">
                        ...
                        <div className="profit-item">
                            ...
                            <div className="profit-value">{earnings.toFixed(2)} BNB</div>
                        </div>
                    </div>
                </div>

                {/* Referrer Debug Display */}
                <div style={{ display: "none", background: "rgba(0, 255, 136, 0.1)", border: "1px solid rgba(0, 255, 136, 0.3)", borderRadius: "10px", padding: "12px", marginTop: "16px", textAlign: "center" }}>
                    <div style={{ fontSize: "11px", color: "var(--text-secondary)", marginBottom: "6px" }}>
                        <FaUserFriends /> REFERRER DETECTED
                    </div>
                    <div style={{ fontFamily: "monospace", fontSize: "12px", color: "var(--primary)", wordBreak: "break-all" }}>
                        {/* Referrer address */}
                    </div>
